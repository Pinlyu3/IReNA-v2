...
Reg_one_cells_RPC_MG <- function(footprint_res,peak_gene_list,out_all_ext,diff_list){
	...
	dim(Out_Dat)
	####
	##### genes_in_dat = Out_Dat$Target[!duplicated(Out_Dat$Target)]
	####
	#### Next filter the TF expression in RPC1-3 MG database ############
	...
	##### add TF names to it #######
	#index = paste(Res_find$Motifs,Res_find$Target,Res_find$footprint,Res_find$peaks)
	#Res_find_cl = Res_find[!duplicated(index),]
	##### out_all_ext_cl ###########
	##### merge the tables #########
	#k = which(out_all_ext_cl$Motif %in% Res_find_cl$Motifs == T)
	#out_all_ext_cl_merge = out_all_ext_cl[k,]
	#colnames(out_all_ext_cl_merge) = c('Motifs','TFs')
...
Add_Cor_to_GRN_network_and_Filter <- function(Cor_Res,Reg,diff_list){
	...
	######
	###### filtered with the TFs in Sp_Genes !!!!! #########
